...
const styles = `
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .msg-input { field-sizing: content; min-height: 20px; max-height: 120px; }
...
const TRANSACTION_STEPS = [
  ...
  { id: 2,  key: "preapproval",         label: "Pre-Approval",      icon: "🏦", owner: "buyer",
    ...
    buyerAction: "Upload your mortgage pre-approval letter OR proof of funds (cash buyers). Required to advance.",
  ...
  { id: 5,  key: "inspection_response", label: "Repair Response",   icon: "🔨", owner: "seller",
    ...
    sellerAction: "Review buyer's repair requests and respond. You may agree, offer a credit, or decline.",
...
const CONTRACT_TEMPLATES = [
  { name: "Purchase & Sale Agreement",    icon: "📋" },
  { name: "Counteroffer Addendum",        icon: "🔄" },
  { name: "Earnest Money Agreement",      icon: "💰" },
  { name: "Property Disclosure Statement",icon: "📝" },
  { name: "Lead Paint Disclosure",        icon: "⚠️" },
  { name: "Inspection Contingency Waiver",icon: "✅" },
  { name: "As-Is Addendum",              icon: "🏚️" },
  { name: "Seller Financing Addendum",    icon: "🏦" },
];
...
async function sendEmail(to, subject, html) {
  if (!to) return;
  try { await fetch("/api/email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ to, subject, html }) }); }
...
function getContractPrompt(templateName, offer) {
  ...
  const p = {
    "Purchase & Sale Agreement": `Generate a complete residential Purchase and Sale Agreement. ${base} Sections: 1)Parties 2)Price 3)Earnest Money 4)Financing Contingency 5)Inspection Contingency 6)Appraisal Contingency 7)Title 8)Closing 9)Possession 10)Inclusions 11)Condition 12)Costs 13)Default 14)Disputes 15)Entire Agreement. Signature blocks. Plain English. No markdown.`,
    "Counteroffer Addendum": `Generate a Counteroffer Addendum. ${base} Counter price: ${offer.counter_price||"___"}. Counter closing: ${offer.counter_closing_date||"___"}. Include reference to original offer, modified terms, expiration, unchanged terms, signatures. No markdown.`,
    ...
    "Property Disclosure Statement": `Generate a Seller Disclosure Statement. ${base} Yes/No/Unknown for: roof, foundation, water, plumbing, electrical, HVAC, pests, environmental, HOA, legal disputes, unpermitted work, defects. Seller certification and signatures. No markdown.`,
    "Lead Paint Disclosure": `Generate a Lead Paint Disclosure per 42 USC 4852d. ${base} Federal requirement, seller checkboxes, records available, EPA pamphlet, 10-day inspection right, signatures. No markdown.`,
    "Inspection Contingency Waiver": `Generate an Inspection Contingency Waiver. ${base} Voluntary waiver, as-is acceptance, no warranties, risks acknowledged, other terms remain, signatures. No markdown.`,
    "As-Is Addendum": `Generate an As-Is Addendum. ${base} As-is statement, no warranties, no repairs or credits, buyer acknowledgment, survivability, signatures. No markdown.`,
    "Seller Financing Addendum": `Generate a Seller Financing Addendum. ${base} Loan amount, interest rate, term, payment, late penalty, default, due-on-sale, security interest, signatures. No markdown.`,
  };
  return p[templateName] || `Generate a professional ${templateName}. ${base} No markdown.`;
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(26,18,8,0.65)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onClose}>
      <div style={{ background: "var(--card)", borderRadius: 24, maxWidth: 420, width: "100%", overflow: "hidden", boxShadow: "0 24px 80px rgba(0,0,0,0.3)" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid var(--warm)" }}>
          {["login","signup"].map(m => (
            <button key={m} onClick={() => { setMode(m); setError(null); setStep(1); }} style={{ flex:1, padding:"12px", background: mode===m ? "var(--card)" : "var(--cream)", border:"none", borderBottom: mode===m ? "2px solid var(--gold)" : "2px solid transparent", fontSize:13, fontWeight: mode===m ? 600 : 400, color: mode===m ? "var(--gold)" : "#888", cursor:"pointer", marginBottom:-1 }}>
        ...
        <div style={{ padding: "22px 28px 26px" }}>
          {mode === "login" ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
              ...
              <div><label style={lbl}>Password</label><input style={inp} type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key==="Enter" && login()} onFocus={e=>e.target.style.borderColor="var(--gold)"} onBlur={e=>e.target.style.borderColor="var(--warm)"} /></div>
  ...
  const submit = async () => {
    ...
    try {
      ...
      await sendEmail(listing.seller_email, "New Offer on "+listing.address,
        "<h2 style='font-family:sans-serif;color:#1a1208'>New offer received!</h2><p style='font-family:sans-serif;color:#444'><strong>"+form.buyer_name+"</strong> offered <strong>"+formatPrice(form.offer_price)+"</strong>. Closing: <strong>"+(form.closing_date||"TBD")+"</strong>. Log in to respond.</p>");
      setSubmitted(true);
...
function SignaturePad({ onSign }) {
  ...
  const getPos = (e, canvas) => { const r = canvas.getBoundingClientRect(); const cx = e.touches?e.touches[0].clientX:e.clientX; const cy = e.touches?e.touches[0].clientY:e.clientY; return { x:cx-r.left, y:cy-r.top }; };
      ...
      {open && (
        <div style={{ marginTop:9, borderRadius:12, overflow:"hidden", border:"1px solid var(--warm)", animation:"fadeIn 0.2s ease" }}>
          ...
          <div style={{ background:"#fff", padding:"8px 10px", borderTop:"1px solid var(--warm)", display:"flex", gap:8, alignItems:"flex-end" }}>
            <textarea ref={inputRef} className="msg-input" value={input} onChange={e=>setInput(e.target.value)}
              onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();} }}
              placeholder="Type a message..." rows={1}
              style={{ flex:1, padding:"9px 14px", borderRadius:22, border:"1.5px solid #e0e0e0", background:"var(--msg-bg)", fontSize:14, outline:"none", resize:"none", fontFamily:"inherit", lineHeight:1.5, color:"var(--ink)", overflowY:"auto" }}
              onFocus={e=>e.target.style.borderColor="var(--sage)"}
              onBlur={e=>e.target.style.borderColor="#e0e0e0"} />
            <button onClick={send} disabled={sending||!input.trim()}
              style={{ background:input.trim()?"var(--sage)":"#ddd", color:"#fff", border:"none", borderRadius:"50%", width:36, height:36, cursor:input.trim()?"pointer":"default", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0, transition:"background 0.15s", boxShadow:input.trim()?"0 2px 8px rgba(74,103,65,0.35)":"none" }}>
...
function StepCard({ step, offer, user, onUpdate, isExpanded, onToggle }) {
  ...
  const advance = async () => {
    ...
    if (!error) {
      ...
      await sendEmail(otherEmail, "Transaction Update: "+step.label+" Complete",
        "<h2 style='font-family:sans-serif;color:#1a1208'>"+step.label+" complete.</h2><p style='font-family:sans-serif;color:#444'>Next: <strong>"+nextStep?.label+"</strong>. Log in to continue.</p>");
  ...
  const submitCounter = async () => {
    ...
    const { error } = await sb.from("offers").update(updates).eq("id", offer.id);
    if (!error) { await sendNotification(offer.buyer_id, "The seller countered your offer.", "offers"); onUpdate({ ...offer, ...updates }); setShowCounter(false); }
    setLoading(false);
  };
  ...
  const handleClosing = async () => {
    setLoading(true);
    const buyerField = "step_closing_buyer_signed";
    const sellerField = "step_closing_seller_signed";
    const myField = isBuyer ? buyerField : sellerField;

    // Update my signature
    await sb.from("offers").update({ [myField]: true }).eq("id", offer.id);

    // Check if both are now signed
    const { data: fresh } = await sb.from("offers")
      .select("step_closing_buyer_signed, step_closing_seller_signed")
    ...
    if (buyerDone && sellerDone) {
      // Both signed — complete the transaction
      await sb.from("offers").update({ status:"closed", step_index:10 }).eq("id", offer.id);
      // Mark listing as sold — RLS now allows offer participants to do this
      const { error: soldErr } = await sb.from("listings")
        .update({ sold:true, sold_at:new Date().toISOString() })
        .eq("id", offer.listing_id);
      if (soldErr) console.error("Sold update error:", soldErr);
      const otherId = isSeller?offer.buyer_id:offer.seller_id;
    ...
    <div style={{ background:"var(--card)", border:"2px solid var(--gold)", borderRadius:12, padding:"15px", marginBottom:9, animation:"fadeIn 0.3s ease" }}>
      ...
      {step.key==="earnest"&&isBuyer&&(
        <div style={{ background:"#fffbf0", border:"1px solid #f0d080", borderRadius:8, padding:"10px 12px", marginBottom:10, fontSize:11, color:"#555", lineHeight:1.8 }}>
          ...
          2. Contact them with the address, price, and both names.<br/>
      ...
      {step.key==="title"&&isSeller&&(
        <div style={{ background:"#f5f0ff", border:"1px solid #c0a0f0", borderRadius:8, padding:"10px 12px", marginBottom:10, fontSize:11, color:"#444", lineHeight:1.8 }}>
          ...
          2. Resolve any liens, unpaid mortgages, or judgments found.<br/>
      ...
      {step.key==="closing"&&(
        <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:7 }}>
            {[["Buyer",offer.step_closing_buyer_signed],["Seller",offer.step_closing_seller_signed]].map(([role,signed])=>(
              <div key={role} style={{ background:signed?"#f0fff4":"var(--cream)", border:"1px solid "+(signed?"#9ae6b4":"var(--warm)"), borderRadius:8, padding:"9px", textAlign:"center" }}>
                <div style={{ fontSize:9, color:"#666", marginBottom:3, textTransform:"uppercase" }}>{role}</div>
                <div style={{ fontSize:12, fontWeight:600, color:signed?"var(--sage)":"#bbb" }}>{signed?"✓ Signed":"Awaiting"}</div>
              </div>
            ))}
          </div>
...
function OffersTab({ user, onRequireAuth }) {
  ...
  const loadUnread = async () => {
    const { count } = await sb.from("notifications")
      .select("id", { count:"exact" }).eq("user_id", user.id).eq("read", false);
...
function MessagesTab({ newThread, user, onRequireAuth }) {
  ...
  const loadConversations = async () => {
    ...
    data.filter(m=>m.user_id!==m.recipient_id).forEach(msg => {
      ...
      if (!map[key]) {
        map[key] = { key, listing_id:msg.listing_id, listing:msg.listings, other_user_id:otherId, other_name:msg.user_id===user.id?msg.recipient_name:msg.sender_name, last_message:msg.body, last_time:msg.created_at, unread:0 };
      }
  ...
  useEffect(() => {
    if (newThread&&user&&newThread.user_id!==user.id) {
      const conv = { key:newThread.id+"-"+[user.id,newThread.user_id].sort().join("-"), listing_id:newThread.id, listing:newThread, other_user_id:newThread.user_id, other_name:newThread.seller_name, last_message:"", last_time:new Date().toISOString(), unread:0 };
      openConv(conv);
    }
  }, [newThread, user]);
        ...
        <div style={{ flex:1, overflow:"auto" }}>
          ...
          {!loading&&conversations.length===0&&(
            <div style={{ padding:"40px 20px", textAlign:"center", color:"#aaa" }}>
              ...
              <div style={{ fontSize:12, lineHeight:1.6 }}>Tap "Message" on any listing to start a conversation with the seller.</div>
            </div>
          )}
      ...
      <div className={"messenger-chat"+(mobileView==="chat"?" active":"")}
        ...
        {activeConv ? (
          <>
            ...
            {/* Messages area */}
            <div style={{ flex:1, overflow:"auto", padding:"16px 18px", display:"flex", flexDirection:"column", gap:2 }}>
              {messages.length===0&&(
              ...
              {groupedMessages.map((item, idx) => {
                if (item.type==="date") return (
                  <div key={"date-"+idx} style={{ display:"flex", justifyContent:"center", margin:"10px 0 6px" }}>
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", color:"#aaa", padding:40 }}>
            ...
            <div style={{ fontSize:13, color:"#888", textAlign:"center", maxWidth:260, lineHeight:1.7 }}>
              Select a conversation to get started, or tap Message on any listing to begin talking with a buyer or seller.
            </div>
          </div>
        )}
      </div>
...
function SellTab({ user, onRequireAuth }) {
  ...
  if (!user) return (
    <div style={{ maxWidth:440, margin:"80px auto", textAlign:"center", padding:"0 24px" }}>
      ...
      <p style={{ color:"#666", lineHeight:1.7, marginBottom:22, fontSize:13 }}>Create a free account and list your home in minutes. Keep more of what you earn — no agent commissions.</p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (id) { setDeepLinkListingId(id); setTab("Browse"); window.history.replaceState({},""," /"); }
  }, []);
